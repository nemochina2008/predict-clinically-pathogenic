works_with_R("3.2.2", data.table="1.9.7")
...
set.means[, rank := rank(mean.error), by=.(train.name, test.name)]
model.ranks <- set.means[, list(
  mean.rank=mean(rank)
  ), by=.(model.name, n.features)]
levs <- model.means[order(mean.error), model.name]
levs <- model.ranks[order(mean.rank), rev(model.name)]
model.colors <- paste(matrix(hue_pal()(length(levs)), nrow=4, byrow=TRUE))
names(model.colors) <- levs
model.colors[["major.class"]] <- "grey"
model.colors[["ctree"]] <- "black"
...
ggplot()+
  ...
  scale_color_manual(values=feature.colors)+
  geom_point(aes(error.percent, model.fac, color=n.features),
             shape=1,
             data=pred.dt)

seg.dt <- pred.dt[, data.table(
  x=min(100-error.percent),
  xend=max(100-error.percent),
  model.fac=factor(levs, levs),
  model.name=levs)]
ggplot()+
  theme_bw()+
...
ggplot()+
  theme_bw()+
  theme(panel.margin=grid::unit(0, "lines"))+
  facet_grid(train.name ~ test.name)+
  scale_color_manual(values=feature.colors)+
  geom_point(aes(auc, model.name, color=n.features),
             shape=1,
             data=pred.dt)
