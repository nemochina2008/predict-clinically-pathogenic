    ...
    for(weight.name in names(weight.list)){
      ...
      for(test.name in names(BP.other)){
        ...
        for(model.name in names(pred.mat.list)){
          ...
          auc <- WeightedAUC(roc)
          BP.roc.list[[paste(model.name, test.name, train.name, test.fold, weight.name)]] <-
            data.table(model.name, test.name, train.name, test.fold, weight.name,
                       roc)
          BP.pred.list[[paste(model.name, test.name, train.name, test.fold, weight.name)]] <-
            data.table(model.name, test.name, train.name, test.fold, weight.name,
                       auc, pred.dot)
        }#model.name
      }#test.name
